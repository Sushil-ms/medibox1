...
const Temperature = () => {
  ...
  const [bodyinfluxData, setBodyInfluxData] = useState([]);
  ...
  useEffect(() => {
    ...
    const influxQuery = async () => {
      ...
      const queryApi = await new InfluxDB({ url, token }).getQueryApi(org);
      //make query
      await queryApi.queryRows(query1, {
        next(row, tableMeta) {
          const o = tableMeta.toObject(row);
          //push rows from query into an array object
          res.push(o);
        ...
        complete() {
          ...
          setBodyChartData(finalData);

          setBodyInfluxData(res);
        },
        error(error) {
          console.log('query failed- ', error);
        },
